...
def run_mrsi_to_t1(config, subject: str, session: str | None, mrsi_reference: Path, t1_path: Path, fixed_mask: Path | None = None) -> MRSIToT1Result:
    prefix = ants_transform_prefix(config.derivative_dir / "transforms" / "ants", subject, session, "mrsi")
    ...
    register(t1_path, mrsi_reference, prefix, transform="sr", fixed_mask=fixed_mask, verbose=config.verbose)
